# Voice

**Status: not yet picked.** Try a few in the dashboard and commit the choice
here once it feels right.

## Selection criteria

The duck has a personality. The voice should support it, not fight it:

- **Slightly dry.** Not chipper. Not customer-service. The duck has been
  watching prints for a while.
- **Casual cadence.** Not announcer-y. Conversational.
- **Younger to mid-range.** A grandparent voice doesn't fit a rubber duck.
- **Not famous.** Avoid voices that strongly evoke a specific person — pulls
  attention away from the duck itself.
- **Latency matters.** Use a Convai-supported voice with low TTFB; some custom
  cloned voices have noticeably higher first-byte latency.

## Candidates to try

(Add ratings as you audition them.)

| Voice ID | Name | Notes |
|---|---|---|
| _tbd_ | _tbd_ | _tbd_ |

## Selected

**Voice ID:** _tbd_
**Model:** `eleven_turbo_v2_5` (or whatever Convai recommends for low-latency conversational)
**Stability:** _tbd_ (lower = more expressive, higher = more consistent)
**Similarity boost:** _tbd_

## Why

_(fill in once selected — what made this one win over the others)_
