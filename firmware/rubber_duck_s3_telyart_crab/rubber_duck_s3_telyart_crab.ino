// ============================================================
// DUCK DUCK DUCK — ESP32-S3 Telyart Crab
// ============================================================
// Telyart ESP32-S3 Crab + MAX98357 I2S DAC + ICS-43434 mic + servo.
...
//
// Board: Telyart ESP32-S3 Crab (select ESP32S3 Dev Module in Arduino)
//
// Wiring:
//   D0  → MAX98357 LRC (WS)
//   D1  → MAX98357 BCLK
...
extern int demoStep;
extern unsigned long lastRealActivity;
...
void setup() {
  ...
  if (Serial) {
    Serial.println("[duck] Ready. Telyart Crab ESP32-S3 + MAX98357");
  ...
  #if ENABLE_BUTTON
  {
    ...
    if (pressed && !buttonDown) {
      ...
      if (!longPressConsumed && held > 50) {
        lastRealActivity = now;  // Any button press resets idle escalation
        if (calibrationMode) {
          advanceCalibration();
        } else if (demoMode) {
