...
- Every prompt you receive and every response you generate is scored on creativity, soundness, ambition, elegance, and risk. By default this uses Apple Foundation Models on-device (~3B, free, sub-second). Optionally switches to Anthropic API (Codex Haiku) for higher-quality scoring.
...
- `bambu/AGENTS.md` — the standalone Bambu Duck (ESP32-S3, ElevenLabs ConvAI via Fly.io relay). No runtime overlap with the Mac duck.
- `boyband/AGENTS.md` — one-time live performance rig: four Bambu Ducks on stage as a boy band. Reuses Bambu firmware unchanged via a local "fake relay" Stage app. Branch `feature/boy-band`.
...
- **Stop** (`scripts/on-Codex-stop.sh`) — sends your response to the eval service
...
- `scripts/` — hook scripts that connect Codex to the eval service
...
- **Mac App Store sandbox**: The widget must work under App Sandbox. It may ONLY read/write files inside `~/Library/Application Support/DuckDuckDuck/` (via FileManager container APIs). It must NEVER access `~/.Codex/`, `~/Documents/`, `~/Library/Preferences/`, or any path outside its sandbox container. All state (API key, PID, logs, session timestamps) belongs in Application Support. The only way to detect the plugin is via the `/health` HTTP ping — never read Codex's settings files. Network outbound from the widget is limited to:
  - `localhost:3333` — own MiniServer
  - `api.anthropic.com` — Codex eval
  - `generativelanguage.googleapis.com` — Gemini eval
  - `api.openai.com` — OpenAI eval
  - `api.github.com` — widget update check (`UpdateChecker`)
  - `ideo.github.io` — firmware manifest + web flasher hand-off (`FirmwareUpdater`)

  Browser-handoff URLs via `NSWorkspace.shared.open` are not widget network traffic and are unrestricted.
...
- Full session: `./scripts/duck-session` (tmux with Codex; widget must be running)
- The widget's right-click menu has "Start Codex Session" to launch a terminal Codex in tmux.
...
```
- `U` = user prompt evaluated, `C` = Codex response evaluated
